...
-- TODO need to run it in symulation and veryfie that the IV and Key actuly uppdate.
library ieee;
...
begin
	...
	-- Add user logic here
        ...
        key <= s01_axis_tdata(255 downto 0);
    
        update_key_iv : process(axis_aclk)
            begin
                if rising_edge(axis_aclk) then
                    if (s01-axis_tvalid = '1') and (ready = '1') then --TODO add the ready signal from the chacha
                        keylen_sig <= '1';
                        iv_reg <= s01_axis_tdata(320 downto 256);
                    end if;
                end if;                
	...
	crypto : chacha_core
		port map(
			...
			key            => x"0001000100010001000100010001000100010001000100010001000100010001",
			keylen         => keylen_sig,
			iv             => x"0001000100010001",
			ctr            => (others => '0'),
			rounds         => "01000",
